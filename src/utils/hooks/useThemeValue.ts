...
function getThemeByName(name: ThemeValue): [Theme, boolean] {
  switch (name) {
    ...
    case 'green':
      return [createMuiTheme({ palette: { type: 'light' } }, zhCN), false];
    case 'light':
      return [createMuiTheme({ palette: { type: 'light' } }, zhCN), false];
    case 'yellow':
      return [createMuiTheme({ palette: { type: 'light' } }, zhCN), false];
...
export function useThemeValue(): [Theme, boolean] {
  ...
  const themeValue = React.useMemo(() => {
    if (typeof settingTheme === 'string') {
      return getThemeByName(settingTheme);
    } else {
      if (isDark) {
        return getThemeByName(settingTheme.dark);
      } else {
        return getThemeByName(settingTheme.light);
      }
    }
  }, [isDark, settingTheme]);
  return themeValue;
